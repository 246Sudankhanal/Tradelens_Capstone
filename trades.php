...
<div class="toolbar">
    ...
    <div class="filters">
        ...
        <input type="date" class="form-control" id="filter-from" onchange="loadTrades()" title="Start Date">
        <input type="date" class="form-control" id="filter-to"   onchange="loadTrades()" title="End Date">
...
<div class="modal-backdrop" id="trade-modal">
    <div class="modal trade-modal-card">
        <div class="modal-header trade-modal-header">
            <div>
                <div class="trade-modal-kicker" id="modal-kicker">New entry</div>
        ...
        <div class="modal-body">
            ...
            <form id="trade-form" novalidate>
                ...
                <input type="hidden" name="trade_type" id="f-type" value="">
                <input type="hidden" name="emotion" id="f-emotion" value="">

                <div class="trade-section">
                    <div class="trade-section-label"><i class="fa-solid fa-compass"></i> Setup</div>
                    <div class="form-grid">
                       <div class="form-group">
    <label class="form-label">Asset *</label>
    <select name="asset_name" id="f-asset" class="form-control" required>
        ...
        <optgroup label="Forex & Crypto">
            ...
            <option value="BTCUSD">BTCUSD</option>
                        ...
                        <div class="form-group full-width">
                            ...
                            <div class="segmented">
                                <button type="button" class="segmented-btn buy" data-type="Buy" onclick="setTradeType('Buy')">
                                    <i class="fa-solid fa-arrow-trend-up"></i> Buy / Long
                                </button>
                                <button type="button" class="segmented-btn sell" data-type="Sell" onclick="setTradeType('Sell')">
                                    <i class="fa-solid fa-arrow-trend-down"></i> Sell / Short
                                </button>
                            </div>
                        </div>
                    </div>
                ...
                <div class="trade-section">
                    <div class="trade-section-label"><i class="fa-solid fa-scale-balanced"></i> Prices & size</div>
                    <div class="form-grid">
                        <div class="form-group">
                        ...
                        <div class="form-group">
                            <label class="form-label">Live P&amp;L</label>
                            <div class="pnl-preview-card" id="pnl-preview-card">
                                <div class="pnl-preview-meta">Estimated result<br><strong id="pnl-direction-label">Select a direction</strong></div>
                                <div class="pnl-preview-value" id="pnl-preview">$0.00</div>
                            </div>
                        </div>
                    </div>
                </div>
                ...
                <div class="trade-section">
                    <div class="trade-section-label"><i class="fa-solid fa-brain"></i> Psychology</div>
                    <div class="emotion-chips" id="emotion-chips">
                        <button type="button" class="emotion-chip" data-emotion="" onclick="setEmotion('')">None</button>
                        <button type="button" class="emotion-chip" data-emotion="Confident" onclick="setEmotion('Confident')">Confident</button>
                        <button type="button" class="emotion-chip" data-emotion="Calm" onclick="setEmotion('Calm')">Calm</button>
                        <button type="button" class="emotion-chip" data-emotion="Patient" onclick="setEmotion('Patient')">Patient</button>
                        <button type="button" class="emotion-chip" data-emotion="Fearful" onclick="setEmotion('Fearful')">Fearful</button>
                        <button type="button" class="emotion-chip" data-emotion="Greedy" onclick="setEmotion('Greedy')">Greedy</button>
                        <button type="button" class="emotion-chip" data-emotion="Impulsive" onclick="setEmotion('Impulsive')">Impulsive</button>
                        <button type="button" class="emotion-chip" data-emotion="Uncertain" onclick="setEmotion('Uncertain')">Uncertain</button>
                    </div>
                </div>

                <div class="trade-section">
                    <div class="trade-section-label"><i class="fa-solid fa-pen-to-square"></i> Notes</div>
...
function setEmotion(emotion) {
    document.getElementById('f-emotion').value = emotion;
...
function openModal(trade = null) {
    document.getElementById('modal-kicker').textContent = trade ? 'Update entry' : 'New entry';
...
function updatePnlPreview() {
    ...
    let qty     = parseFloat(document.getElementById('f-qty').value)   || 1;
    const type  = document.getElementById('f-type').value;
    const asset = document.getElementById('f-asset').value.toUpperCase();
    const el    = document.getElementById('pnl-preview');
    const card  = document.getElementById('pnl-preview-card');
    const label = document.getElementById('pnl-direction-label');

    // Define custom multipliers or contract sizing rules per asset class
    let multiplier = 1;
    if (asset.includes('XAUUSD') || asset.includes('GOLD')) {
        multiplier = 100; // 1 lot = 100 troy ounces
    } else if (asset.includes('XAGUSD') || asset.includes('SILVER')) {
        multiplier = 5000; // Silver standard contract is usually 5,000 oz
    } else if (asset.includes('US30') || asset.includes('NAS100')) {
        multiplier = 1; // 1 contract per point (adjust based on your broker specs)
    }

    let pnl = 0;
    if (type === 'Buy')  pnl = (exit - entry) * qty * multiplier;
    if (type === 'Sell') pnl = (entry - exit) * qty * multiplier;

    el.textContent = (pnl >= 0 ? '+' : '') + '$' + Math.abs(pnl).toFixed(2);
    el.style.color = pnl > 0 ? 'var(--green)' : pnl < 0 ? 'var(--red)' : '';
    card.classList.toggle('is-win', pnl > 0);
    card.classList.toggle('is-loss', pnl < 0);
    label.textContent = type === 'Buy' ? 'Long position' : type === 'Sell' ? 'Short position' : 'Select a direction';
}
...
async function submitTrade() {
    ...
    if (!document.getElementById('f-type').value) {
        errEl.textContent = 'Please choose Buy / Long or Sell / Short.';
